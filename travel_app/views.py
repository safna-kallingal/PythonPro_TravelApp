...
class UserTravelPackageListView(generics.ListAPIView):
    serializer_class = TravelPackageSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return TravelPackage.objects.filter(created_by=self.request.user)
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                response_data = {
                    ...
                    'user_id': user.id,  # Add the user ID to identify the logged-in user
                }
